...
@router.post("/project/{project_id}/batch-generate", response_model=BatchGenerateResponse, summary="??????????")
...
@router.get("/batch-generate/{batch_id}/status", response_model=BatchGenerateStatusResponse, summary="??????????")
...
@router.get("/batch-generate/{batch_id}/stream", summary="????/???????????")
...
@router.get("/project/{project_id}/batch-generate/active", summary="????????????????")
...
@router.get("/batch-generate/active-tasks", summary="???????????????")
...
@router.post("/batch-generate/{batch_id}/cancel", summary="????????")
async def cancel_batch_generation(
    ...
    if task.status in ['completed', 'failed', 'cancelled']:
        raise HTTPException(status_code=400, detail=f"????? {task.status} ???????")
...
@router.post("/batch-generate/{batch_id}/resume", summary="????????")
