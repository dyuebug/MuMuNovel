...
from tests.test_support.database_test_support import check_database_health, close_db
...
def build_default_startup_status() -> dict:
    return {
        ...
        "completed_at": None,
        "steps": {
            "mcp_status_sync": {"healthy": False, "status": "pending", "detail": "waiting"},
...
def create_lifespan(state: dict):
    ...
    async def lifespan(app: FastAPI):
        ...
        mark_startup_step(
            ...
            "mcp_status_sync",
            healthy=True,
            detail="python mcp startup sync retired; rust mcp plugin owner active",
        )
        ...
        mark_startup_step(
            ...
            "background_tasks",
            healthy=True,
            detail="python startup load retired; rust background task owner active",
        )
