...
from app.services.chapter_generation_stream_candidate_service import (
...
from app.services.chapter_generation_stream_execution_service import (
...
from app.services.chapter_generation_stream_finalize_service import (
        ...
        if db_session:
            try:
                if not db_committed and db_session.in_transaction():
                    await db_session.rollback()
                    logger.warning("Rolled back dangling transaction in chapter stream finally block")

                await db_session.close()
                logger.info("Chapter stream DB session closed")
            except Exception as close_error:
                logger.error(f"Failed to close chapter stream DB session: {close_error}")
                try:
                    await db_session.close()
                except Exception:
                    pass
