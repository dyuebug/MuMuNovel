...
- 保持与 schema 中定义的完全一致，包括大小写和命名风格"""
    ...
    # 灵感模式 - 书名生成（系统提示词）
    INSPIRATION_TITLE_SYSTEM = """你是懂网文市场的创作搭档。
...
1. 贴住原始想法，不跑题
2. 和书名气质一致
3. 每个50-100字，语言自然，别端着
4. 都要带核心冲突或悬念
5. 方向可以不同，但都要能落回同一个故事内核
...
请补全：
1. title: 书名（3-6字；用户已给则保留）
2. description: 简介（50-100字；紧贴已给信息）
3. theme: 核心主题（30-50字；前后一致）
4. genre: 类型标签数组（2-3个）

要求：补全内容要像同一个项目里写出来的，语气自然，不跑偏。
