...
`backend/app/api/chapters.py` ??????????? / seam gateway??????? route ???????????????????????
??????????wiring?????????? facade????

- batch/run ?? facade??? `execute_batch_generation_in_order(...)`
- single chapter generation facade??? `generate_single_chapter_for_batch(...)`
...
???? `chapters.py` ??????????????????????? gateway / seam ??? ???
???????????

- ????????????????????? import ? facade
- ?? wrapper ???????????? seam ?????????

???????????????? `chapters_api.*` ?? monkeypatch ?????????? service?????????????
