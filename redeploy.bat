...
echo Usage: redeploy.bat [-NoCache] [-UseCnMirror] [-SkipFrontendBuild] [-FullRestart] [-NoPause^|-NonInteractive] [-RepairPostgresPassword]
echo.
echo redeploy.bat is a compatibility entrypoint for deploy-strangler.bat.
set "PS_HOST_FLAGS="
if defined NO_PAUSE_FLAG set "PS_HOST_FLAGS=-NonInteractive"
powershell.exe %PS_HOST_FLAGS% -NoProfile -ExecutionPolicy Bypass -File "%~dp0redeploy.ps1" %*
