...
from migrator_app.models.project_default_style import ProjectDefaultStyle
from migrator_app.models.writing_style import WritingStyle
...
_sync_low_ai_presets_lock = asyncio.Lock()
_sync_low_ai_presets_done = False


async def sync_low_ai_presets(
    db: "AsyncSession",
    force: bool = False,
) -> bool:
    """同步 low_ai 预设到最新定义。"""
    global _sync_low_ai_presets_done
    if _sync_low_ai_presets_done and not force:
        return False

    async with _sync_low_ai_presets_lock:
        if _sync_low_ai_presets_done and not force:
            return False
        ...
        _sync_low_ai_presets_done = True
        return modified


async def resolve_project_default_style_id(
    db_session: "AsyncSession",
    project_id: str,
) -> Optional[int]:
    result = await db_session.execute(
        select(ProjectDefaultStyle.style_id).where(
            ProjectDefaultStyle.project_id == project_id
        )
    )
    return result.scalar_one_or_none()


async def resolve_effective_style_context(
    *,
    db_session: "AsyncSession",
    user_id: str,
    project_id: str,
    ...
    if prefer_project_default_style and not resolved_style_id:
        resolved_style_id = await resolve_project_default_style_id(
            db_session,
            project_id,
        )
    ...
    if style.user_id is not None and style.user_id != user_id:
        logger.warning(
            f"⚠️ {log_prefix} - 风格 {resolved_style_id} 不属于当前用户，跳过"
        )
        return context
...
async def resolve_chapter_quality_profile(
    *,
    ...
    normalized_reference_assets = tuple(
        reference_assets or normalized_external_assets or ()
    )
    ...
    if project:
        profile.update(
            await resolve_effective_style_context(
    ...
    if enable_mcp and user_id:
        from tests.test_support.ai_gateway.mcp_tools_loader import mcp_tools_loader
        ...
        try:
            ...
            logger.warning(
                f"⚠️ {log_prefix} - 获取MCP参考块失败，已回退为空: {mcp_error}"
            )
...
def clone_chapter_quality_profile(
    ...
    *,
    ...
    if reference_assets is None:
        normalized_reference_assets = tuple(
            external_assets or base_reference_assets or ()
        )
    else:
