"""Prompt template sync owner for managed template metadata and legacy auto-sync."""
...
class TemplateSyncRule:
    ...
    def is_legacy_hash(self, content_hash: str) -> bool:
        return (
            bool(content_hash)
            and content_hash in self.legacy_hashes
            and content_hash not in self.current_hashes
        )
...
def build_template_sync_status(
    *,
    ...
    user_template: Optional[Any] = None,
) -> Dict[str, Any]:
    ...
    if not system_template_content:
        return {
            ...
            "user_content_hash": (
                calculate_content_hash(user_template.template_content)
                if user_template
                else None
            ),
...
async def sync_managed_template_if_legacy(
    *,
    ...
    rule = MANAGED_TEMPLATE_SYNC_RULES.get(template_key)
    if not rule or not system_template_content:
        return False
    ...
    if system_template_info:
        ...
        if system_parameters is not None:
            user_template.parameters = json.dumps(
                system_parameters,
                ensure_ascii=False,
            )
