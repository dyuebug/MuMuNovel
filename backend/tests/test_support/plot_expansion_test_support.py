...
@lru_cache(maxsize=1)
def _load_plot_expansion_prompt_template_map() -> dict[str, str]:
    ...
    for template_key in template_keys:
        ...
        if not match:
            raise RuntimeError(
                f"plot expansion test support 未找到模板常量: {template_key}"
            )
        templates[template_key] = match.group(1)
    return templates


def _plot_expansion_template_lookup(template_key: str) -> Optional[str]:
    return _load_plot_expansion_prompt_template_map().get(template_key)


async def _default_get_plot_expansion_template(
    template_key: str,
    user_id: str,
    db: AsyncSession,
):
    return await get_template_for_owner(
        ...
        user_id,
        db,
        template_lookup=_plot_expansion_template_lookup,
    )


def _default_format_plot_expansion_prompt(template: str, **kwargs) -> str:
    return _facade_format_prompt(template, **kwargs)


class PromptService:
    get_template = staticmethod(_default_get_plot_expansion_template)
    format_prompt = staticmethod(_default_format_plot_expansion_prompt)


async def get_template(*args, **kwargs):
    return await _default_get_plot_expansion_template(*args, **kwargs)


def format_prompt(*args, **kwargs):
    return _default_format_plot_expansion_prompt(*args, **kwargs)
...
async def _get_plot_expansion_template(template_key: str, user_id: str, db: AsyncSession):
    patched_impl = globals().get("get_template")
    if patched_impl is None:
        raise RuntimeError("plot expansion get_template 未定义")
    return await patched_impl(template_key, user_id, db)


def _format_plot_expansion_prompt(template: str, **kwargs) -> str:
    patched_impl = globals().get("format_prompt")
    if patched_impl is None:
        raise RuntimeError("plot expansion format_prompt 未定义")
...
class PlotExpansionService:
    ...
    async def _generate_chapters_single_batch(
        ...
        # 获取自定义提示词模板
        template = await _get_plot_expansion_template("OUTLINE_EXPAND_SINGLE", project.user_id, db)
        # 格式化提示词
        prompt = _format_plot_expansion_prompt(
            template,
...
"""
            # 获取自定义提示词模板
            template = await _get_plot_expansion_template("OUTLINE_EXPAND_MULTI", project.user_id, db)
            # 格式化提示词
            prompt = _format_plot_expansion_prompt(
                template,
                project_title=project.title,
