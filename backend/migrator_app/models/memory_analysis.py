"""Story memory and plot analysis ORM models."""
...
class StoryMemory(Base):
    """Persisted story memory rows owned by the shared model layer."""
...
class PlotAnalysis(Base):
    """Persisted plot analysis rows owned by the shared model layer."""
    ...
    emotional_tone = Column(String(100), comment="主导情感")
    emotional_intensity = Column(Float, comment="情感强度 0.0-1.0")
    emotional_curve = Column(JSON, comment="情感曲线")
    hooks = Column(JSON, comment="钩子列表")
    ...
    suggestions = Column(JSON, comment="改进建议列表")
