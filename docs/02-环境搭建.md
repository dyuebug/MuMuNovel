...
# 编辑配置
echo "VITE_API_BASE_URL=http://localhost:8005" > .env.local
```
...
```bash
# 在项目根目录启动 Rust runtime + Rust db-migrator + Nginx
...
# 启动 Rust runtime + Rust db-migrator + Nginx
deploy-strangler.bat -NonInteractive

# 查看日志
docker compose -f docker-compose.strangler.yml logs -f rust-backend nginx
...
**访问应用**：
- 网关入口：http://localhost:8005
- Rust API：http://localhost:8005/api
