...
async fn update_user(
        ...
        if existing.is_admin {
            ...
            if admin_count <= 1 {
                return Err((
                    ...
                    Json(json!({"detail": "不能取消最后一个管理员的权限"})),
                ));
            }
        }
    }
    ...
    if body.get("avatar_url").is_some() {
        active.avatar_url = Set(body
            .get("avatar_url")
            .and_then(|v| v.as_str())
            .map(String::from));
    }
    if let Some(v) = body.get("trust_level").and_then(|v| v.as_i64()) {
        active.trust_level = Set(v as i32);
    }
    if let Some(v) = body.get("is_admin").and_then(|v| v.as_bool()) {
...
async fn reset_password(
    ...
    let now = Utc::now();
    let existing_pwd = user_password::Entity::find_by_id(&user_id)
        .one(&db)
        .await
        .map_err(|e| {
            (
...
async fn delete_user(
    ...
    if target.is_admin {
        ...
        if admin_count <= 1 {
            return Err((
                StatusCode::BAD_REQUEST,
                Json(json!({"detail": "不能删除最后一个管理员账号"})),
            ));
        }
    }
