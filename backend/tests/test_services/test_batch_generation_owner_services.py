...
from tests.test_support import (
    batch_generation_single_chapter_wiring_test_adapter as batch_candidate_service,
)
from tests.test_support import (
    batch_generation_orchestration_test_adapter as batch_orchestration_service,
)

# Owner-level regression coverage for the retired batch execution facade.
...
def test_should_build_batch_generation_request_payload_with_request_options_and_custom_model():
    ...
    payload = batch_candidate_service.build_batch_generation_request_payload(
        prompt='draft prompt',
        project=project,
...
def test_should_calculate_estimated_time():
    estimated = batch_orchestration_service.calculate_estimated_time(
        chapter_count=3,
...
def test_should_enqueue_batch_generation_execution():
    ...
    batch_orchestration_service.enqueue_batch_generation_execution(
        background_tasks,
        fake_execution,
        batch_id='batch-1',
        user_id='user-1',
...
def test_should_build_batch_generation_candidate_quality_hooks():
    ...
    hooks = batch_candidate_service.build_batch_generation_candidate_quality_hooks(
...
def test_should_build_batch_generation_candidate_runtime_state():
    state = batch_candidate_service.build_batch_generation_candidate_runtime_state(max_candidates=2)
...
async def test_should_create_batch_generation_candidate_execution():
    ...
    execution = batch_candidate_service.create_batch_generation_candidate_execution(
        ai_service=SimpleNamespace(api_provider='openai'),
...
async def test_should_wait_for_batch_generation_candidate_and_emit_progress(monkeypatch):
    ...
    selected_candidate = await batch_candidate_service.wait_for_batch_generation_candidate(
        ...
        db_session=None,
        publish_stream_event_fn=fake_publish,
    )
...
async def test_should_emit_batch_generation_selected_candidate_events(monkeypatch):
    ...
    await batch_candidate_service.emit_batch_generation_selected_candidate_events(
        stream_task_id='task-1',
        stream_chunks=True,
...
def test_should_build_batch_generation_selected_candidate_result():
    ...
    result = batch_candidate_service.build_batch_generation_selected_candidate_result(
        chapter=chapter,
        selected_candidate={
            'full_content': 'part-1\npart-2',
            'word_count': 1234,
...
async def test_should_build_batch_generation_prompt_for_one_to_one_next_with_style():
    ...
    result = await batch_candidate_service.build_batch_generation_prompt(
        db_session=None,
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=3, title='Test Chapter'),
...
async def test_should_build_batch_generation_context_for_one_to_many_with_stats_and_outline_sources():
    ...
    result = await batch_candidate_service.build_batch_generation_context(
        db_session=None,
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=4, title='Test Chapter'),
        project=Project(id='project-1', title='Test Project', user_id='user-1', outline_mode='one-to-many'),
...
async def test_should_prepare_batch_generation_runtime_with_base_quality_profile():
    ...
    result = await batch_candidate_service.prepare_batch_generation_runtime(
        db_session=None,
        user_id='user-1',
        project=Project(id='project-1', title='Test Project', user_id='user-1'),
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=3, title='Test Chapter'),
...
async def test_should_resolve_batch_generation_chapter_runtime():
    ...
    build_calls = []
    finalize_calls = []
    runtime_preparation = batch_candidate_service.BatchGenerationRuntimePreparation(
    ...
    built_context = batch_candidate_service.BatchGenerationBuiltContext(
        chapter_context=SimpleNamespace(chapter_outline='outline'),
        outline_runtime_sources={'outline_id': 'outline-1'},
    )
    resolved_runtime = batch_candidate_service.BatchGenerationResolvedRuntime(
    ...
    result = await batch_candidate_service.resolve_batch_generation_chapter_runtime(
        db_session=None,
        user_id='user-1',
        project=Project(id='project-1', title='Test Project', user_id='user-1'),
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=2, title='Test Chapter'),
...
def test_should_finalize_batch_generation_runtime():
    ...
    result = batch_candidate_service.finalize_batch_generation_runtime(
...
async def test_should_execute_batch_generation_candidate_flow_without_stream():
    ...
    def fake_build_candidate_quality_hooks(**kwargs):
        ...
        return batch_candidate_service.BatchGenerationCandidateQualityHooks(
    ...
    flow_result = await batch_candidate_service.execute_batch_generation_candidate_flow(
        stream_task_id=None,
        stream_chunks=False,
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=2, title='Test Chapter'),
        effective_story_packet=SimpleNamespace(source='prepared'),
...
async def test_should_execute_batch_generation_generation_stage_with_stream_progress():
    ...
    async def fake_execute_candidate_flow(**kwargs):
        ...
        return batch_candidate_service.BatchGenerationCandidateFlowResult(
            selected_candidate={'full_content': 'draft'},
            selected_candidate_result={'word_count': 12},
        )

    result = await batch_candidate_service.execute_batch_generation_generation_stage(
        stream_task_id='task-1',
        stream_chunks=True,
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=4, title='Test Chapter'),
        effective_story_packet=SimpleNamespace(source='prepared'),
...
async def test_should_execute_batch_generation_prompt_stage():
    ...
    async def fake_build_prompt(**kwargs):
        ...
        return batch_candidate_service.BatchGenerationPrompt(
    ...
    def fake_build_request_payload(**kwargs):
        ...
        return batch_candidate_service.BatchGenerationRequestPayload(
            system_prompt='system-prompt',
            max_tokens=1234,
            generate_kwargs={'prompt': kwargs['prompt'], 'temperature': 0.7},
        )

    result = await batch_candidate_service.execute_batch_generation_prompt_stage(
        db_session=None,
        chapter=Chapter(id='chapter-1', project_id='project-1', chapter_number=6, title='Test Chapter'),
        project=Project(id='project-1', title='Test Project', user_id='user-1'),
        chapter_context=SimpleNamespace(chapter_outline='outline'),
