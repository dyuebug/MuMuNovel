"""Test-only chapter candidate targeted final repair support migrated out of app/services."""
...
def attach_repair_seed_candidate_metadata(
    *,
    ...
    candidate_selection = (
        dict(existing_selection) if isinstance(existing_selection, dict) else {}
    )
    candidate_selection["repair_seed_candidate_index"] = max(
        int(repair_seed_candidate.get("candidate_index") or 1),
        1,
    )
    repair_seed_generation_path = str(
        repair_seed_candidate.get("generation_path") or ""
    ).strip()
    repair_seed_attempt_kind = str(
        repair_seed_candidate.get("attempt_kind") or ""
    ).strip()
    if repair_seed_generation_path:
        candidate_selection["repair_seed_generation_path"] = (
