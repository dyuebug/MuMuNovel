...
- 保持与 schema 中定义的完全一致，包括大小写和命名风格"""
    ...
    # 灵感模式 - 书名生成（系统提示词）
    INSPIRATION_TITLE_SYSTEM = """你是网文立项阶段的创作搭档。
用户的原始想法：{initial_idea}

请给出 6 个书名候选，要求：
1. 紧扣原始想法和核心冲突，不要偏题
2. 风格要拉开，至少覆盖不同气质（热血/悬疑/情感/奇诡等）
3. 中文语感要顺口、好记，避免生造词和拗口词
4. 不要使用《》符号，不要批量同构命名
5. 允许有一点网感，但不要硬塞热梗
...
只返回纯JSON，不加任何解释。"""
    ...
    # 灵感模式 - 书名生成（用户提示词）
    INSPIRATION_TITLE_USER = "我的想法是：{initial_idea}\n请给我6个风格有区分的书名候选，只返回JSON。"

    # 灵感模式 - 简介生成（系统提示词）
    INSPIRATION_DESCRIPTION_SYSTEM = """你是懂网文读者口味的创作搭档。
用户的原始想法：{initial_idea}
已确定的书名：{title}

请生成 6 个简介候选，要求：
1. 全部贴住原始想法，不跑题，和书名气质一致
2. 每个 60-130 字，长短句混用，读起来像真人在讲故事
3. 每个选项都要包含：主角当下目标 + 关键阻碍/代价（至少命中其一）
4. 冲突要具体可感，避免“这是一个关于…”“故事围绕…”这类模板开头
5. 出现设定术语时，紧跟一句白话解释（例如“也就是……”）
6. 可少量使用网络语感，但必须贴场景、点到为止
...
只返回纯JSON，不要附加说明。"""
    ...
    # 灵感模式 - 简介生成（用户提示词）
    INSPIRATION_DESCRIPTION_USER = "原始想法：{initial_idea}\n书名：{title}\n请给我6个简介候选，语言自然、别空话，只返回JSON。"

    # 灵感模式 - 主题生成（系统提示词）
    INSPIRATION_THEME_SYSTEM = """你是小说主题策划搭档。
用户的原始想法：{initial_idea}
小说信息：
- 书名：{title}
- 简介：{description}
...
2. 每个 50-140 字，先说清“这本书到底在讲什么”，再落到冲突现场
3. 角度必须区分开（成长、复仇、救赎、探索、秩序与自由等）
4. 用有情绪温度的自然表达，避免空泛口号和论文腔
5. 如果用了抽象术语，补一句人话解释，降低阅读门槛
...
只返回纯JSON，不加其他说明。"""
    ...
    # 灵感模式 - 主题生成（用户提示词）
    INSPIRATION_THEME_USER = "原始想法：{initial_idea}\n书名：{title}\n简介：{description}\n请给我6个主题候选，角度尽量拉开，只返回JSON。"

    # 灵感模式 - 类型生成（系统提示词）
    INSPIRATION_GENRE_SYSTEM = """你是题材定位搭档。
用户的原始想法：{initial_idea}
小说信息：
- 书名：{title}
- 简介：{description}
...
请生成 6 个类型标签候选（每个 2-4 字），要求：
1. 对应用户原始想法中的题材倾向，和整体气质一致
2. 标签之间可组合，但不能互相冲突
3. 优先使用读者常见认知标签，避免生造词
4. 至少覆盖“主赛道标签 + 气质标签”两种维度
...
返回JSON：
{{"prompt":"选几个最贴合的类型标签（可多选）：","options":["类型1","类型2","类型3","类型4","类型5","类型6"]}}

只返回紧凑JSON，不要加解释。"""

    # 灵感模式 - 类型生成（用户提示词）
    INSPIRATION_GENRE_USER = "原始想法：{initial_idea}\n书名：{title}\n简介：{description}\n主题：{theme}\n请给我6个可组合的类型标签候选，只返回JSON。"

    # 灵感模式智能补全提示词
    INSPIRATION_QUICK_COMPLETE = """你是小说立项补全助手。用户给了部分信息，请把缺失字段补完整。

用户已提供的信息：
{existing}

请补全：
1. title: 书名（3-8字；用户已给则保留）
2. description: 简介（70-140字；紧贴已给信息）
3. theme: 核心主题（40-90字；和简介保持同一冲突链）
4. genre: 类型标签数组（2-3个）
