...
def recover_analysis_task_if_needed(task: AnalysisTask) -> bool:
    ...
    if task.status == 'running':
        ...
        if task.started_at and (current_time - task.started_at) > timedelta(minutes=timeout_minutes):
            ...
            task.progress = 0
            auto_recovered = True
            logger.warning('Auto recovered stale running analysis task: %s, chapter=%s', task.id, task.chapter_id)
        ...
        if task.created_at and (current_time - task.created_at) > timedelta(minutes=3):
            ...
            task.progress = 0
            auto_recovered = True
            logger.warning('Auto recovered stale pending analysis task: %s, chapter=%s', task.id, task.chapter_id)
