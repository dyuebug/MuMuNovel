...
pub async fn generate_career_system(
    ...
    *progress.lock().await = 15;
    ...
    let template = match PromptTemplateService::system_template_info("CAREER_SYSTEM_GENERATION") {
        ...
        None => {
            channel
                .error("CAREER_SYSTEM_GENERATION模板未找到", 500)
                .await;
            return;
        }
    };
    ...
    params.insert(
        "location".into(),
        world_data["location"].as_str().unwrap_or("未设定").into(),
    );
    params.insert(
        "atmosphere".into(),
        world_data["atmosphere"].as_str().unwrap_or("未设定").into(),
    );
    params.insert(
        "rules".into(),
        world_data["rules"].as_str().unwrap_or("未设定").into(),
    );
    ...
    while retry_count < MAX_RETRIES && !success {
        ...
        channel
            .progress("生成职业体系中...", 20, "processing")
            .await;
        *progress.lock().await = 20;
        ...
        if accumulated.trim().is_empty() {
            ...
            if retry_count < MAX_RETRIES {
                channel
                    .progress(
                        &format!("⚠ AI返回为空，重试 ({}/{})", retry_count, MAX_RETRIES),
                        *progress.lock().await,
                        "processing",
                    )
                    .await;
        ...
        channel
            .progress("解析职业体系数据...", 82, "processing")
            .await;
        ...
        match serde_json::from_str::<Value>(&cleaned) {
            ...
            Err(_e) => {
                ...
                if retry_count < MAX_RETRIES {
                    channel
                        .progress(
                            &format!("⚠ JSON解析失败，重试 ({}/{})", retry_count, MAX_RETRIES),
                            *progress.lock().await,
                            "processing",
                        )
                        .await;
    ...
    if let Err(e) = ProjectService::update_wizard_step(db, project_id, 2).await {
        channel
            .progress(&format!("⚠ 向导步骤更新失败: {}", e), 95, "processing")
            .await;
    }
...
pub async fn generate_characters(
    ...
    let wctx = world_context.unwrap_or_else(|| {
        serde_json::json!({
            "time_period": project.world_time_period.as_deref().unwrap_or("未设定"),
            "location": project.world_location.as_deref().unwrap_or("未设定"),
            "atmosphere": project.world_atmosphere.as_deref().unwrap_or("未设定"),
    ...
    *progress.lock().await = 10;
        ...
        .await
    {
        ...
        Err(e) => {
            channel
                .progress(&format!("⚠ 加载职业失败: {}", e), 10, "processing")
                .await;
            vec![]
        }
    };
    ...
    let main_careers: Vec<&career::Model> =
        careers.iter().filter(|c| c.career_type == "main").collect();
    let sub_careers: Vec<&career::Model> =
        careers.iter().filter(|c| c.career_type == "sub").collect();
        ...
        if !main_careers.is_empty() {
            ...
            for c in &main_careers {
                careers_context.push_str(&format!(
                    "- {}: {}\n",
                    c.name,
                    c.description.as_deref().unwrap_or("暂无描述")
                ));
            }
        }
    ...
    let career_name_map: HashMap<&str, &career::Model> =
        careers.iter().map(|c| (c.name.as_str(), c)).collect();
    ...
    let template = match PromptTemplateService::system_template_info("CHARACTERS_BATCH_GENERATION")
    {
        ...
        None => {
            channel
                .error("CHARACTERS_BATCH_GENERATION模板未找到", 500)
                .await;
            return;
        }
    };
    ...
    for batch_idx in 0..total_batches {
        ...
        while retry_count < MAX_RETRIES && !batch_success {
            ...
            let batch_req = if batch_idx == 0 {
                if current_batch_size == 1 {
                    ...
                    format!(
                        "{}\n请精确生成{}个角色:1个主角(protagonist)和{}个核心配角(supporting)",
                        req_str,
                        current_batch_size,
                ...
                format!(
                    ...
                    req_str, current_batch_size, existing_context
                )
            } else {
                format!(
            ...
            params.insert(
                "time_period".into(),
                wctx["time_period"].as_str().unwrap_or("未设定").into(),
            );
            params.insert(
                "location".into(),
                wctx["location"].as_str().unwrap_or("未设定").into(),
            );
            params.insert(
                "atmosphere".into(),
                wctx["atmosphere"].as_str().unwrap_or("未设定").into(),
            );
            params.insert(
                "rules".into(),
                wctx["rules"].as_str().unwrap_or("未设定").into(),
            );
            params.insert(
                "theme".into(),
                theme
                    .unwrap_or(project.theme.as_deref().unwrap_or("未设定"))
                    .into(),
            );
            params.insert(
                "genre".into(),
                ...
                match chunk_result {
                    Ok(chunk) => {
                        if let Some(text) = chunk.content {
                            ...
                            if chunk_count % 10 == 0 {
                                let pct = (batch_progress
                                    + (accumulated.len() as u32 / 40).min(45))
                                .clamp(batch_progress, batch_progress + 45);
                                channel
                                    .progress(
                                        &format!(
                                            "生成第{}/{}批角色中... ({}字符)",
            ...
            match parse_character_batch_items(&cleaned) {
                ...
                Err(parse_error) => {
                    ...
                    if retry_count < MAX_RETRIES {
                        ...
                        channel
                            .error(
                                &format!("第{}批JSON解析失败（已达最大重试次数）", batch_idx + 1),
                                500,
                            )
                            .await;
                        return;
        ...
        if !batch_success {
            channel
                .error(
                    &format!("第{}批在{}次重试后仍然失败", batch_idx + 1, MAX_RETRIES),
                    500,
                )
                .await;
            return;
    ...
    let valid_names: Vec<String> = all_characters
        .iter()
        .filter_map(|c| {
            c.get("name")
                .and_then(|v| v.as_str())
        ...
        .collect();
    let valid_name_set: std::collections::HashSet<&str> =
        valid_names.iter().map(|s| s.as_str()).collect();
    let _org_names: std::collections::HashSet<&str> = all_characters
        .iter()
        .filter(|c| {
            c.get("is_organization")
                .and_then(|v| v.as_bool())
    ...
    channel
        .progress("保存角色到数据库...", 78, "processing")
        .await;
    *progress.lock().await = 78;
    ...
    for char_data in &all_characters {
        let name = char_data
            ...
            .and_then(|v| v.as_str())
            .unwrap_or("未命名角色");
        let is_org = char_data
            .get("is_organization")
            .and_then(|v| v.as_bool())
            .unwrap_or(false);

        let traits_str = char_data.get("traits").map(|v| v.to_string());

        // Build relationships text from relationships_array
        let rel_text = char_data
            .get("relationships_array")
            .and_then(|v| v.as_array())
            .map(|rels| {
                let descs: Vec<String> = rels
                    .iter()
                    .map(|r| {
                        let target = r
        ...
        match CharacterService::create_full(
            ...
            char_data
                .get("age")
                .and_then(|v| v.as_str())
                .or_else(|| char_data.get("age").and_then(|v| v.as_i64()).map(|_| "0")),
        ...
        *progress.lock().await = 83;
        ...
        for (char_model, char_data) in &created {
            ...
            if let Some(ca) = char_data.get("career_assignment") {
                ...
                let main_id = main_name
                    .and_then(|n| career_name_map.get(n))
                    .map(|c| c.id.as_str());
                let sub_list: Vec<Value> = ca
                    .get("sub_careers")
                    .and_then(|v| v.as_array())
                    .map(|a| {
                        a.iter()
                            .filter_map(|s| {
                                ...
                                career_name_map
                                    .get(sn)
                                    .map(|c| serde_json::json!({"career_id": c.id, "stage": stage}))
                            })
                            .collect()
                    })
                    .unwrap_or_default();

                let sub_json = if sub_list.is_empty() {
                    None
                } else {
                    Some(serde_json::to_string(&sub_list).unwrap_or_default())
                };
    ...
    *progress.lock().await = 88;
    ...
    for (char_model, char_data) in &created {
        ...
        {
            Ok(org) => {
                org_name_to_id.insert(name.clone(), org.id);
            }
            Err(e) => {
                channel
                    .progress(
                        &format!("⚠ 创建组织失败: {} - {}", name, e),
                        *progress.lock().await,
                        "processing",
                    )
                    .await;
    ...
    *progress.lock().await = 93;
    ...
    channel
        .progress(&format!("已创建{}条关系", rel_count), 95, "processing")
        .await;

    // --- Update project ---
    if let Err(e) = ProjectService::update_wizard_step(db, project_id, 3).await {
        channel
            .progress(&format!("⚠ 向导步骤更新失败: {}", e), 96, "processing")
            .await;
    }
...
pub async fn generate_outline(
        ...
        .await
    {
        Ok(c) => c,
        Err(e) => {
            channel.error(&format!("加载角色失败: {}", e), 500).await;
            return;
        }
    };
    ...
    channel
        .progress(
            &format!("准备生成{}个大纲节点...", chapter_count),
            5,
            "processing",
        )
        .await;
    let template = match PromptTemplateService::system_template_info("OUTLINE_CREATE") {
        Some(t) => t,
        None => {
            channel.error("加载大纲模板失败", 500).await;
            return;
        }
    };
    ...
    params.insert(
        ...
        narrative_perspective.unwrap_or("").to_string(),
    );
    ...
    params.insert(
        "creative_mode".into(),
        creative_mode.unwrap_or("").to_string(),
    );
    ...
    params.insert(
        ...
        story_creation_brief.unwrap_or("").to_string(),
    );
    params.insert(
        "quality_preset".into(),
        quality_preset.unwrap_or("").to_string(),
    );
    params.insert(
        "quality_notes".into(),
        quality_notes.unwrap_or("").to_string(),
    );
    ...
    channel
        .progress("AI正在生成大纲...", 10, "processing")
        .await;
    ...
    // --- Save outlines to DB ---
    channel
        .progress("保存大纲到数据库...", 60, "processing")
        .await;
    ...
    for (i, item) in outline_data.iter().take(limit).enumerate() {
        ...
        .await
        {
            ...
            Ok(None) => {
                channel.error("无权创建大纲", 403).await;
                return;
            }
            Err(e) => {
                channel.error(&format!("保存大纲失败: {}", e), 500).await;
                return;
            }
        }
    }
    ...
    channel
        .progress(
            &format!("已创建{}个大纲节点", created_outlines.len()),
            70,
            "processing",
        )
        .await;
    ...
    if outline_mode == "one-to-one" {
        channel
            .progress("一对一模式：自动创建章节...", 75, "processing")
            .await;
        ...
        for (i, (outline, _)) in created_outlines.iter().enumerate() {
            ...
            match ChapterService::create_pending(db, project_id, &outline.title, chapter_number)
                .await
            {
                Ok(ch) => created_chapters.push(ch),
                Err(e) => {
                    channel
                        .progress(&format!("⚠ 创建章节失败: {}", e), 80, "processing")
                        .await;
                }
            }
        }
        channel
            .progress(
                &format!("已自动创建{}个章节", created_chapters.len()),
                85,
                "processing",
            )
            .await;
        ...
        channel
            .progress("细化模式：跳过自动创建章节", 85, "processing")
            .await;
    }
    ...
    {
        channel
            .progress(&format!("⚠ 项目更新失败: {}", e), 92, "processing")
            .await;
    }

    // --- Result ---
    let note = if outline_mode == "one-to-one" {
        "已自动创建章节"
    } else {
        "可在大纲页面手动展开"
    };
    let outline_items: Vec<Value> = created_outlines
        .iter()
        .map(|(m, _)| {
            serde_json::json!({
                "id": m.id,
    ...
    let chapter_items: Vec<Value> = created_chapters
        .iter()
        .map(|c| {
            serde_json::json!({
                "id": c.id,
    ...
    let result_message = if outline_mode == "one-to-one" {
        format!(
            "成功生成{}个大纲节点并自动创建{}个章节（传统模式）",
            created_outlines.len(),
            created_chapters.len()
        )
    } else {
        format!(
            "成功生成{}个大纲节点（细化模式，可在大纲页面手动展开）",
            created_outlines.len()
        )
    };
