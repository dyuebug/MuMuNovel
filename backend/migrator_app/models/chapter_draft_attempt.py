"""Chapter draft attempt ORM model."""
...
class ChapterDraftAttempt(Base):
    """Persisted chapter draft attempt rows owned by the shared model layer."""
    ...
    batch_task_id = Column(
        String(36),
        ForeignKey("batch_generation_tasks.id", ondelete="SET NULL"),
        nullable=True,
        comment="批量任务ID",
    )
