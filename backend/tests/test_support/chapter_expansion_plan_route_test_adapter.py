...
from tests.test_support.chapter_schema_test_support import ExpansionPlanUpdate

router = APIRouter(prefix="/chapters", tags=["章节管理"])
logger = get_logger(__name__)


async def update_chapter_expansion_plan_with_default_route_wiring(
    *,
    chapter_id: str,
    ...
    if plan_data:
        if chapter.expansion_plan:
            try:
                ...
                logger.warning(f"章节 {chapter_id} 的 expansion_plan 格式错误,将覆盖")
