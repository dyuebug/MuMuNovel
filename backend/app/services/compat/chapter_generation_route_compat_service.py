...
async def generate_chapter_content_stream_with_default_route_wiring(
    *,
    ...
    return await generate_chapter_content_stream_with_default_wiring(
        ...
        one_to_one_builder_cls=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["OneToOneContextBuilder"],
        ).OneToOneContextBuilder,
        one_to_many_builder_cls=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["OneToManyContextBuilder"],
        ).OneToManyContextBuilder,
        get_template_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["get_template"],
        ).get_template,
        format_prompt_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["format_prompt"],
        ).format_prompt,
        apply_style_to_prompt_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["apply_style_to_prompt"],
        ).apply_style_to_prompt,
        build_runtime_system_prompt_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["build_chapter_runtime_system_prompt"],
        ).build_chapter_runtime_system_prompt,
        detect_style_profile_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["detect_style_profile"],
        ).detect_style_profile,
        resolve_generation_temperature_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["resolve_generation_temperature"],
        ).resolve_generation_temperature,
        compute_story_quality_metrics_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["compute_story_quality_metrics"],
        ).compute_story_quality_metrics,
        resolve_quality_gate_execution_plan_fn=__import__(
            "app.services.chapter_generation_route_compat_service",
            fromlist=["resolve_quality_gate_execution_plan"],
        ).resolve_quality_gate_execution_plan,
