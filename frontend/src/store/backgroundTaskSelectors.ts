...
import { isActiveBackgroundTask, type TrackedBackgroundTask } from './backgroundTaskModel';
import type { BackgroundTaskMap } from './backgroundTaskStateHelpers';

export type BackgroundTaskStatusPriority = Record<BackgroundTaskRuntimeStatus, number>;

export const selectBackgroundTaskList = (tasksMap: BackgroundTaskMap): TrackedBackgroundTask[] =>
...
export const selectActiveBackgroundTaskCount = (tasksMap: BackgroundTaskMap): number =>
  selectActiveBackgroundTasks(selectBackgroundTaskList(tasksMap)).length;

export const selectVisibleBackgroundTasks = (
  tasksMap: BackgroundTaskMap,
...
export const selectBackgroundTaskSections = (
  tasks: TrackedBackgroundTask[],
  focusProjectId: string | null,
  taskFilter: 'overview' | 'active' | 'current' | 'failed',
): Array<{
  key: string;
  title: string;
  description: string;
