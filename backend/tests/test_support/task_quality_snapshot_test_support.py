...
async def record_task_quality_metrics(
    ...
    async with _task_quality_lock:
        ...
        if normalized_event and not isinstance(normalized_event.get("repair_guidance"), dict):
            normalized_event["repair_guidance"] = build_story_repair_guidance(
                normalized_event,
                scope="chapter",
            )
        if normalized_event and not isinstance(normalized_event.get("quality_gate"), dict):
            normalized_event["quality_gate"] = build_quality_gate_decision(
                normalized_event,
                scope="chapter",
            )
        ...
        current["summary"] = build_quality_metrics_summary_from_state(
            summary_state,
            scope="batch",
        )
...
async def get_task_quality_metrics_snapshot(
    ...
    recovered_history = _normalize_quality_payload(
        persisted_snapshot.quality_metrics_history
    ) or []
    summary_state = (
        build_quality_metrics_summary_state(recovered_history, scope="batch")
    ...
    if not recovered_snapshot["summary"] and summary_state is not None:
        recovered_snapshot["summary"] = build_quality_metrics_summary_from_state(
            summary_state,
            scope="batch",
        )
