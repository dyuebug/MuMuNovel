"""Test-only support for retired chapter generation runtime prompt assembly."""
...
def _split_outline_sentences(text: str) -> list[str]:
    parts = re.split(r"[。！？!?；;\n]+", text)
    return [part.strip() for part in parts if part.strip()]


def _extract_outline_anchor_lines(
    ...
    raw_lines = [
        line.strip() for line in chapter_outline.splitlines() if line.strip()
    ]
    ...
    for line in raw_lines:
        ...
        if any(key in cleaned for key in keywords):
            parts = [
                part.strip() for part in re.split(r"[:：]", cleaned, maxsplit=1)
            ]
            if len(parts) == 2 and parts[1] and any(
                key in parts[0] for key in keywords
            ):
    ...
    for sentence in _split_outline_sentences(chapter_outline):
...
def build_chapter_runtime_system_prompt(
    ...
    if isinstance(story_runtime_contract, dict):
        ...
        if isinstance(runtime_blueprint, dict):
            organization_ledger_source = (
                runtime_blueprint.get("organization_state_ledger") or []
            )
        if not organization_ledger_source:
            organization_ledger_source = (
                story_runtime_contract.get("organization_state_ledger") or []
            )
    ...
    safe_target_word_count = (
        max(200, int(target_word_count or 0)) if target_word_count else None
    )
    target_lower_bound = (
        max(
            200,
            min(
                safe_target_word_count - 120,
                int(safe_target_word_count * 0.9),
            ),
        )
    ...
    target_upper_bound = (
        max(
            ...
            min(
                safe_target_word_count + 150,
                int(safe_target_word_count * 1.15),
            ),
        )
        if safe_target_word_count
        else None
