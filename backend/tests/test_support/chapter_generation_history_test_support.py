...
def build_generation_history_payload(
    ...
    *,
    ...
    from tests.test_support.schemas.generation_payload import build_chapter_generation_quality_history_payload
...
def _collect_candidate_runtime_items(
...
def _append_candidate_quality_token(tokens: List[str], seen: set[str], token: str, *, max_tokens: int) -> None:
    ...
    if (
        ...
        or normalized in _CANDIDATE_QUALITY_STOPWORDS
...
def _truncate_candidate_quality_snippet(
    snippet: str,
    *,
...
def _build_candidate_draft_quality_highlights(
    *,
    ...
    continuity_matched, continuity_missing, continuity_evidence = _split_candidate_quality_item_matches(
        normalized_content,
        continuity_items,
    )
    ...
    foreshadow_matched, foreshadow_missing, foreshadow_evidence = _split_candidate_quality_item_matches(
        normalized_content,
        foreshadow_items,
    )
